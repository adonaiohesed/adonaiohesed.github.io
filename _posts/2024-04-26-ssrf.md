...
image: "/assets/thumbnails/2024-04-26-ssrf.png"
bilingual: true
date: 2024-04-26 14:09:36
---
...
### **심층 분석: 암묵적 신뢰의 원인 (Why Implicit Trust Exists)**

SSRF가 단순한 리다이렉션 이슈를 넘어 치명적인 취약점이 되는 핵심 이유는 애플리케이션이 **로컬 머신(localhost)에서 발생하는 요청을 외부 요청과 다르게 취급하고 암묵적으로 신뢰하기 때문**입니다. 왜 애플리케이션은 이러한 방식으로 설계되어 공격의 빌미를 제공하는 것일까요? 주요 원인은 다음과 같습니다.

* **아키텍처에 의한 접근 제어 우회:** 종종 접근 제어(Access Control) 로직이 애플리케이션 서버 내부가 아닌, 그 앞단에 위치한 다른 구성 요소(예: 로드 밸런서, WAF, 리버스 프록시)에 구현되어 있는 경우가 있습니다. 서버가 자기 자신(Loopback)으로 연결을 시도할 때, 트래픽은 이 앞단 구성 요소를 거치지 않으므로 접근 제어 확인이 완전히 우회됩니다.
* **재해 복구(Disaster Recovery)를 위한 예외 처리:** 관리자가 자격 증명을 분실하거나 인증 서버에 장애가 발생했을 때를 대비하여, 로컬 머신(localhost)에서 접속하는 사용자에게는 로그인 절차 없이 관리자 권한을 허용하도록 시스템이 구성되기도 합니다. 이는 오직 물리적으로 서버에 접근 가능한 신뢰된 관리자만이 로컬에서 접속할 것이라는 가정을 전제로 하지만, SSRF는 원격 공격자가 로컬 요청인 것처럼 위장할 수 있게 하여 이 가정을 무너뜨립니다.
* **비공개 포트의 관리자 인터페이스:** 관리자 인터페이스나 민감한 모니터링 도구가 메인 애플리케이션과 다른 포트(예: 8080, 9000)에서 수신 대기하며, 외부망에서는 접근이 불가능하도록 설정된 경우가 많습니다. 사용자는 직접 접근할 수 없지만, 로컬 서버 내부에서는 해당 포트로의 접근이 제한되지 않으므로 SSRF를 통해 이러한 숨겨진 인터페이스에 도달할 수 있습니다.

이러한 특수한 신뢰 관계들로 인해, 로컬 머신에서 시작된 것으로 보이는 요청은 일반적인 요청보다 훨씬 높은 권한을 갖거나 보안 검사를 우회하게 되며, 이것이 바로 SSRF가 치명적인 이유입니다.
...
* **블랙리스트 우회 기법 및 IP 난독화 (IP Obfuscation):**
    단순한 문자열 필터링을 우회하기 위해 IP 주소의 다양한 표기법을 악용할 수 있습니다.
    * **검증과 실행의 불일치:** 방어 로직(WAF 등)은 입력값을 단순한 '문자열'로 비교하여 차단하지만, 실제 요청을 수행하는 백엔드(운영체제 및 네트워크 라이브러리)는 이를 유효한 IP 주소로 '해석'하여 연결을 시도하기 때문에 발생합니다.
    * **옥텟 생략 (Octet Omission):** `127.1`과 같은 표기가 가능한 이유는 운영체제가 4개 미만의 옥텟이 입력될 경우, 마지막 숫자를 남은 비트 전체로 해석하기 때문입니다. (예: `127(8bit)` + `1(24bit)` $\rightarrow$ `127.0.0.1`)
    * **다양한 표기법 변형:** IP 주소는 본질적으로 32비트 정수이므로 다음과 같이 다양하게 표현해도 컴퓨터는 `127.0.0.1`과 동일하게 인식합니다.
        | 표기법 | 입력값 예시 | 설명 |
        | :--- | :--- | :--- |
        | **표준 표기** | `127.0.0.1` | 일반적인 점-10진수 표기법 |
        | **옥텟 생략** | `127.1` | 중간의 0을 생략 (127.0.0.1로 해석됨) |
        | **정수 표기** | `2130706433` | IP 전체를 하나의 10진수 정수로 변환 |
        | **8진수** | `0177.0.0.1` | 127을 8진수 0177로 표현 |
        | **16진수** | `0x7f000001` | IP 전체를 16진수로 변환 |
...
해당 IP 난독화 기법 외에, SSRF 공격에서 자주 사용되는 DNS Rebinding 공격이나 특정 클라우드 벤더(AWS, Azure)별 구체적인 SSRF 페이로드 예시도 필요하시다면 말씀해 주세요.
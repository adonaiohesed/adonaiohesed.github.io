...
image: "/assets/thumbnails/2024-12-16-ids_ips.png"
date: 2024-12-16 06:28:48
bilingual: true
---
...
**침입 탐지 시스템(IDS)**은 네트워크나 시스템에 대한 악의적인 행위나 정책 위반, 즉 **침입을 탐지하여 관리자에게 보고(Reporting)하고 기록(Logging)하는 장치**입니다.
...
보안 아키텍처를 구성할 때 보통 **인터넷 → 방화벽 → IPS → 내부 네트워크** 순서로 배치합니다. 방화벽이 1차로 불필요한 트래픽을 걸러내고, IPS가 그 뒤에서 방화벽을 통과한 트래픽의 내용까지 정밀하게 검사하여 보안 효율을 극대화하는 것입니다.
...
*   <span style="color:red">False Positive (FP)</span>: **'과잉 방어'**입니다. 정상적인 활동을 공격으로 오인하여 경보를 울리거나 차단합니다. Too many FPs can lead to alert fatigue for administrators, causando them to miss real threats. (Type I Error)
...
*   이 지표가 낮으면 미탐(FN)이 많다는 의미이며, 실제 공격을 놓치고 있다는 뜻이므로 매우 위험합니다.
*   $$\text{Recall} = \frac{TP}{TP + FN}$$
...
*   가장 직관적인 지표이지만, 데이터가 불균형할 때 성능을 왜곡할 수 있어 맹신해서는 안 됩니다.
*   $$\text{Accuracy} = \frac{TP + TN}{TP + TN + FP + FN}$$

### ROC Curve와 AUC
*   **ROC (Receiver Operating Characteristic) 곡선**은 재현율과 오탐율의 관계를 시각적으로 표현한 그래프입니다.
*   그래프가 **좌측 상단에 가까울수록** 더 좋은 성능의 모델을 의미합니다.
*   **AUC (Area Under the Curve)**는 이 ROC 곡선 아래의 면적을 의미하며, 1에 가까울수록 성능이 뛰어난 모델입니다.
...
현대적인 보안 환경에서는 **심층 방어(Defense-in-Depth)** 전략이 필수적입니다. 여러 보안 계층이 서로의 약점을 보완하며 시너지를 낼 때 가장 강력한 방어 체계를 구축할 수 있습니다.
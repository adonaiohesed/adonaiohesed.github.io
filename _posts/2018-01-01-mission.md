...
## Introduce
I have 2 years of security engineer & researcher and did application security reviews, pen testing, threat modeling, and development of automated detection tools. 

My backgroud is cyber security as master degree and computer science as bachelor. Most recently I worked at CableLabs and developed a detection system for BGP hijacking attacks. Because BGP essentially assumes all networks are trustworthy, there is vulnerlable. No security safeguards have yet been incorporated globaally that can validate routes are authentic, it has vulnerability to be attakced by using legitimate network prefix. Unfortunately, BGP hijacking can be diffcult to detect as the owner of the network prefix is unable to detect the hijack themselves. So my solution help to detect it to our clients by monitoring whole network. The solution is anomaly-based IDS. Thanks to this solution, the clients can detect the hijacking by my monitoring system and prevent from DDoS attack or lost of their trafic.

Prior to that, I worked for Cisco on an open source project that educate general software engineers about Cryptography. Not all engineers are able to use proper cryptography yet. The goal of this project was to properly train such engineers. I provide documnetation of cryptography to technical stakeholders such as what vulnerability of MD5, what is TLS and how to implement it.

When I was working at eloicube in Korea, I created a firewall management solution. In Korea, the majority of banks and insurance companies employed only a small number of network managers. Therefore, the network managers could not efficiently manage all firewalls. My solution helped them to efficiently manage all the firewalls. It help to manage history of settings/rule changes of all firewalls, detecte expired policies, over-permitted polices, and long-term non-use polices for optimization to easy to manage the firewalls. As a result, effective management can be achieved with fewer network managers. Also, during this project period, I have experience issued a certificate for TLS setting.

The career I talked about mainly mentioned things related to the blue team's job, but I also have experience with the red team's job during the Johns Hopkins University master programe, including pen testing, threat modeling, trickbot malware reproduction, and CTF participation.

In the security field, there is still a lot to learn, the learning process is so enjoyable for me. I want to find a good place which gatered good colleagues and continue to discover vulnerabilities and apply remidation and mitigation to actual products.

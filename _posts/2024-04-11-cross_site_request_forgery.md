...
### 1. Using Referer and Origin Headers

  * **Referer Header:** The HTTP request header includes a `Referer` field, which indicates the URL of the previous web page from which the request originated. Your server can check this `Referer` header to determine if the request came from your own domain or from another website (cross-site).
  * **Origin Header:** Modern browsers also send the `Origin` header for state-changing requests (like POST). Unlike `Referer`, it only contains the scheme, host, and port without the full path. **If the `Origin` header is missing or does not match your domain, the server should block the request.** This is a critical server-side verification because attackers cannot easily forge these headers in a standard browser environment.
  * **Drawbacks:** Some privacy tools or older browsers might strip these headers. However, for a defense-in-depth approach, validating both headers provides a strong baseline for identifying cross-site requests.

### 2. Same-Site Cookies

  * `Same-Site` cookies are a powerful defense mechanism where the **server and browser share responsibility.** 
      * **Server's Role:** The server **directs** the defense by adding the `SameSite` attribute to the `Set-Cookie` header.
      * **Browser's Role:** The browser **enforces** the defense by deciding whether to include the cookie based on the request's context (Same-site vs Cross-site).
  * The `Same-Site` attribute can have one of three values:
      * **`Strict`:** The cookie is sent **only with same-site requests.** It is not sent with any cross-site requests.
      * **`Lax`:** The cookie is sent with same-site requests and top-level navigation (like clicking a link). It is **not sent** with sub-resource requests (images, iframes) or cross-site POST requests.
      * **`None`:** The cookie is sent with all cross-site requests. Must be used with the `Secure` attribute.
...
### 4. Same-Origin Policy (SOP)
  ...
  * The **Same-Origin Policy (SOP)** is a browser-level security gate. However, it is vital to understand its specific limitation: **SOP blocks "Read" but not "Write".**
  * **How it helps:** SOP prevents a malicious script on `evil.com` from **reading** the data returned from your server (`bank.com`). This is why an attacker cannot simply "steal" your CSRF token by reading your page content via JavaScript.
  * **Where it fails (CSRF):** SOP does **not** stop the browser from **sending** (Writing) a request to another origin. When a user clicks a malicious link, the browser happily sends the POST request along with your cookies to `bank.com`. The server processes the request, and the damage is done; the malicious site just can't read the response. **This is why CSRF is possible even with SOP in place.**
...
### 1. Referer 및 Origin 헤더 사용 (`Referer` & `Origin` Headers)

  * **Referer 헤더:** 요청이 시작된 이전 페이지의 URL을 담고 있습니다. 서버는 이를 확인하여 요청이 자신의 도메인에서 시작되었는지 검증할 수 있습니다.
  * **Origin 헤더:** 최신 브라우저는 POST와 같은 상태 변경 요청 시 `Origin` 헤더를 함께 보냅니다. `Referer`와 달리 상세 경로를 제외한 도메인 정보(프로토콜, 호스트, 포트)만 담고 있습니다. **서버 측에서 이 `Origin` 헤더가 없거나 자신의 도메인과 일치하지 않는 경우 요청을 차단해야 합니다.** 이는 공격자가 브라우저 환경에서 임의로 조작하기 어렵기 때문에 매우 효과적인 서버 측 검증 수단입니다.
  * **단점:** 일부 프라이버시 도구나 구형 브라우저에서 헤더가 누락될 수 있습니다. 따라서 심층 방어(Defense in Depth)를 위해 다른 대책과 병행하는 것이 좋습니다.

### 2. Same-Site 쿠키 (`Same-Site` Cookies)

  * `Same-Site` 쿠키는 **서버와 브라우저가 역할을 분담**하는 강력한 방어 기제입니다.
      * **서버의 역할:** `Set-Cookie` 헤더에 `SameSite` 속성을 추가하여 방어 방향을 **지시(Direct)**합니다.
      * **브라우저의 역할:** 서버의 지시에 따라 크로스 사이트 요청 시 쿠키를 포함할지 말지를 결정하고 **집행(Enforce)**합니다.
  * 세 가지 속성 값의 의미:
      * **`Strict`:** 동일 사이트 요청에서만 쿠키 전송.
      * **`Lax`:** 동일 사이트 요청 및 링크 클릭(Top-level navigation) 시 전송. 크로스 사이트 POST 요청 등에서는 **전송되지 않음.**
      * **`None`:** 모든 요청에 전송 (반드시 `Secure`와 병행).
...
### 4. Same-Origin Policy (동일 출처 정책, SOP)
  ...
  * **동일 출처 정책(SOP)**은 브라우저의 기본 보안 관문입니다. 하지만 핵심적인 한계를 이해해야 합니다: **SOP는 '읽기(Read)'는 막지만 '쓰기(Write/Send)'는 막지 못합니다.**
  * **도움이 되는 부분:** SOP는 `evil.com`의 스크립트가 여러분의 서버(`bank.com`)에서 반환된 데이터를 **읽는 것**을 차단합니다. 이 덕분에 공격자가 자바스크립트로 여러분의 페이지를 읽어 CSRF 토큰을 탈취하는 것이 불가능합니다.
  * **한계 (CSRF가 가능한 이유):** SOP는 브라우저가 다른 출처로 요청을 **보내는 것(Write)** 자체는 막지 않습니다. 사용자가 악성 링크를 누르면, 브라우저는 아무 문제 없이 `bank.com`으로 POST 요청과 쿠키를 보냅니다. 서버는 이를 처리하고 작업(이체 등)을 완료합니다. 악성 사이트가 그 결과를 '읽지' 못할 뿐, **이미 서버에서는 작업이 수행되었으므로 공격은 성공한 것입니다.**
